 ...
 * @returns {Boolean}
 */
function isValidSelector(selector, componentName, isStrictMode, opts) {
  var selectors = opts.selectors;

  // Don't bother with :root
  if (selector === ':root') { return true; }

  var initialPattern = (selectors.initial) ?
    selectors.initial(componentName, opts.namespace) :
    selectors(componentName, opts.namespace);
  var chainablePattern = (selectors.chainable) ?
    selectors.chainable(componentName, opts.namespace) :
    initialPattern;
